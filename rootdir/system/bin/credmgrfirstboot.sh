...
CREDMGRCNT=$(find $CREDFOLDER -group oem_2996 -type f -name "credmgr.db" | wc -l )

echo -e "credmgr folder:\n$(ls -la $CREDFOLDER)\ncredmgr match (if any):" >> $LOG
find $CREDFOLDER -group oem_2996 -type f -name "credmgr.db" >> $LOG
...
if [ "x$CREDMGRCNT" == "x0" ]; then
	...
	chown system:oem_2996 $CREDFOLDER >> $LOG 2>&1
